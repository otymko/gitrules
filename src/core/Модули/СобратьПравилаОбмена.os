...
Функция ЗагрузитьКаталогиВДеревоПравил(ДеревоКаталогов)
	...
	Если ИмяКорневогоУзла = "ПравилаОбмена" Или ИмяКорневогоУзла = "ПравилаРегистрации" Тогда
		...
		ЗагрузитьРеквизитыКонвертации(КорневойУзел, УзелПравил);
		ЗагрузитьСобытияКонвертации(ДеревоКаталогов, УзелПравил);
		Для Каждого СтрокаДерева Из ДеревоКаталогов.Строки Цикл
			Если СтрокаДерева.Имя = "Параметры" Тогда
				ЗагрузитьПараметры(СтрокаДерева, УзелПравил);
			ИначеЕсли СтрокаДерева.Имя = "Обработки" Тогда 
				ЗагрузитьОбработки(СтрокаДерева, УзелПравил);
			ИначеЕсли СтрокаДерева.Имя = "ПравилаКонвертацииОбъектов" Тогда
				ЗагрузитьПравилаКонвертацииОбъектов(СтрокаДерева, УзелПравил);
			ИначеЕсли СтрокаДерева.Имя = "ПравилаВыгрузкиДанных" Тогда
				ЗагрузитьПравилаВыгрузкиДанных(СтрокаДерева, УзелПравил);
			ИначеЕсли СтрокаДерева.Имя = "ПравилаОчисткиДанных" Тогда
				ЗагрузитьПравилаОчисткиДанных(СтрокаДерева, УзелПравил);
			ИначеЕсли СтрокаДерева.Имя = "Алгоритмы" Тогда
				ЗагрузитьАлгоритмы(СтрокаДерева, УзелПравил);
			ИначеЕсли СтрокаДерева.Имя = "Запросы" Тогда
				ЗагрузитьЗапросы(СтрокаДерева, УзелПравил);
			ИначеЕсли СтрокаДерева.Имя = "СоставПланаОбмена" Тогда
				ЗагрузитьСоставПланаОбмена(СтрокаДерева, УзелПравил);
			ИначеЕсли СтрокаДерева.Имя = "ПравилаРегистрацииОбъектов" Тогда
				ЗагрузитьПравилаРегистрацииОбъектов(СтрокаДерева, УзелПравил);
			Иначе
...
Процедура ЗагрузитьРеквизитыКонвертации(ТекущийУзел, КорневойУзел)
...
Процедура ЗагрузитьСобытияКонвертации(ДеревоКаталогов, КорневойУзел)
...
Процедура ЗагрузитьПараметры(ДеревоКаталогов, КорневойУзел)
...
Процедура ЗагрузитьОбработки(ДеревоКаталогов, КорневойУзел)
...
Процедура ЗагрузитьПравилаКонвертацииОбъектов(ДеревоКаталогов, КорневойУзел)
...
Процедура ЗагрузитьПравилаВыгрузкиДанных(ДеревоКаталогов, КорневойУзел)
...
Процедура ЗагрузитьПравилаОчисткиДанных(ДеревоКаталогов, КорневойУзел)
...
Процедура ЗагрузитьАлгоритмы(ДеревоКаталогов, КорневойУзел)
...
Процедура ЗагрузитьЗапросы(ДеревоКаталогов, КорневойУзел)
...
Процедура ЗагрузитьСоставПланаОбмена(ДеревоКаталогов, КорневойУзел)
...
Процедура ЗагрузитьПравилаРегистрацииОбъектов(ДеревоКаталогов, КорневойУзел)
...
Процедура ЗагрузитьТаблицаСвойств(ИмяТаблицы, ДеревоСвойств, КорневойУзел)
	...
	Для Каждого СтрокаДерева Из ТекущийУзел.Строки Цикл
		...
		Если МожноСортировать Тогда
			Если ИмяПоиска = "ГруппаПравило" И СтрокаДерева.Имя = "Правило" Тогда
				// Для сохранения исходной сортировки файла правил регистрации
...
Процедура РекурсивноПрочитатьЭлементыИГруппыОтбораПоСвойствам(КорневойУзел, ТекущаяСтрокаДерева)
	...
	ЗагрузитьТаблицаСвойств("ТаблицаСвойствОбъекта", ТекущаяСтрокаДерева, ТекущийУзел);
	ЗагрузитьТаблицаСвойств("ТаблицаСвойствПланаОбмена", ТекущаяСтрокаДерева, ТекущийУзел);
...
Процедура РекурсивноПрочитатьКаталогиГруппИПравил(КорневойУзел, ТекущаяСтрокаДерева)
	...
	Если ИмяУзла = "Правило" И ОбщийФункционал.ПроверитьНаличиеПредкаВДереве(ТекущийУзел, "ПравилаКонвертацииОбъектов") Тогда
