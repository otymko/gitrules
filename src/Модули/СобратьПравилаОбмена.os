...
Процедура ВыполнитьСборку(Знач ПутьККаталогуПравил) Экспорт
	
	ЕстьОшибки = Ложь;
	ИнициализироватьИмена();	
	ДеревоФайлов = ПолучитьДеревоФайловПоКаталогу(Новый Файл(ПутьККаталогуПравил));
	...
	Сообщить("Создан файл: " + ИмяВременногоФайла);	
КонецПроцедуры
...
Процедура ОбработатьОтступыВXML(ПутьКФайлу)
	...
	ЗаписьТекста.Записать(ИмяВремФайла);
